...
def wi(shape, name):
    # init using a normal distribution
    # TODO: He init used here?
    return tf.Variable(tf.random_normal(shape, name=name)) / math.sqrt(shape[0])

# in Python 3 generator is one-time used, hence we convert it to a list
...
weights = {'W': [wi(s, "W") for s in shapes],  # Encoder weights
           ...
           # batch normalization parameter to shift the normalized value (bias term)
           'beta': [bi(0.0, layer_sizes[l+1], "beta") for l in range(L)],
           # batch normalization parameter to scale the normalized value (scaling term)
...
# now we can use tf.Tensor.getitem instead
# X's shape is [num_sampels, num_features], where first batch_size elements are labeled
...
def update_batch_normalization(batch, l):
    ...
    # use ewma.apply to maintain average
...
def encoder(inputs, noise_std):
    ...
    for l in range(1, L+1):
        print("Layer ", l, ": ", layer_sizes[l-1], " -> ", layer_sizes[l], sep='')
        # h is (corrupted) hidden representation from the previous layer (after activation)
        ...
        def training_batch_norm():
            ...
            if noise_std > 0:
                ...
                # use labeled samples that go through the cleaner encoder to compute batch norm for evaluation step
        ...
        # we need mean and variance of z_pre for computing loss function
        d['labeled']['h'][l], d['unlabeled']['h'][l] = split_lu(h)
    return h, d

print("=== Corrupted Encoder ===")
# y_c is softmax output (corrupted)
y_c, corr = encoder(inputs, noise_std)

print("=== Clean Encoder ===")
# y is softmax output (clean)
...
for l in range(L, -1, -1):
    # notice that only in training step we can use batch normalization
    print("Layer ", l, ": ", layer_sizes[l+1] if l+1 < len(layer_sizes) else None, " -> ", layer_sizes[l], ", denoising cost: ", denoising_cost[l], sep='')
    z, z_c = clean['unlabeled']['z'][l], corr['unlabeled']['z'][l]
    m, v = clean['unlabeled']['m'].get(l, 0), clean['unlabeled']['v'].get(l, 1-1e-10)
    if l == L:
        u = unlabeled(y_c)
    else:
        # reconstruct by using information from upper layer of the decoder
        # by the assumption of conditional independence
        u = tf.matmul(z_est[l+1], weights['V'][l])
    u = batch_normalization(u)
    z_est[l] = g_gauss(z_c, u, layer_sizes[l])
    z_est_bn = (z_est[l] - m) / v
    # append the cost of this layer to d_cost
    # implement Eq. (3) in the arXiv paper
...
# bool label of correct prediction?
...
if ckpt and ckpt.model_checkpoint_path:
    ...
    # saving is done after epoch epoch_n is finished
    i_iter = (epoch_n+1) * (num_examples // batch_size)
    print("Restored Epoch ", epoch_n, sep='')
else:
...
print("=== Training ===")
print("Initial Accuracy: ", sess.run(accuracy, feed_dict={inputs: mnist.test.images, outputs: mnist.test.labels, training: False}), "%", sep='')
...
print("Final Accuracy: ", sess.run(accuracy, feed_dict={inputs: mnist.test.images, outputs: mnist.test.labels, training: False}), "%", sep='')
